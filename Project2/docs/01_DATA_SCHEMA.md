...
struct ItemDataComponent {
    ...
    // ← 新增：磁吸免疫计时器（爆米花效果）
    float magnetImmunityTimer{0.25f};  // 0.25 秒内不被磁吸
};
```

---

## 八、战利品与掉落组件 (Loot & Drop Components)
...
struct LootEntry {
    ...
    // 磁吸参数（已废弃，现在 MagnetComponent 在玩家身上）
    float magnetRadius{0.0f};    // ⚠️ 已废弃：保留兼容性
    float magnetSpeed{400.0f};   // ⚠️ 已废弃：保留兼容性
};
```

### 8.2 LootDropComponent
...
> **⚠️ 关键设计：** 挂载在**玩家**身上，而非掉落物！
> 
> 玩家拥有"吸收半径"，掉落物进入半径后飞向玩家。

```cpp
struct MagnetComponent {
    float magnetRadius{200.0f};  // 吸收半径（像素）
    float magnetSpeed{400.0f};   // 吸收速度（像素/秒）
};
```
...
## 九、临时实体组件 (Temporary Entity Components)

### 9.1 LifetimeComponent
...
**生命周期：**
1. DamageSystem 挂载 DeathTag
2. LootSpawnSystem 读取并生成掉落物
3. DeathSystem 销毁实体

---

## 十、攻击状态组件 (Attack State Components)

### 10.1 AttackStateComponent
...
struct AttackStateComponent {
    ...
    bool hitActivated{false};   // 是否已激活 Hitbox（单次锁）
};
```

**设计说明：**
- `hitActivated` 确保每次攻击只创建一次 Hitbox
- 进入 Attack 状态时添加
- 离开 Attack 状态时移除

---

## 十一、环境与区域组件 (Environment Components)

### 11.1 BiomeZoneComponent
...
## 十二、渲染事件组件 (Render Events)

### 12.1 RenderEventComponent
...
| 日期 | 改动 | 负责人 |
|------|------|--------|
| 2026-04-03 | 添加战利品组件（LootDrop/Magnet/PickupBox/Lifetime/DeathTag/AttackState） | AI Assistant |
| 2026-04-03 | 更新 ItemDataComponent 添加 magnetImmunityTimer 字段 | AI Assistant |
